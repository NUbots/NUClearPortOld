    ...
    namespace math {
        namespace kalman {
            ...
            arma::vec::fixed<IMUModel::size> IMUModel::timeUpdate(const arma::vec::fixed<size>& state, double deltaT) {
                ...
                const double omega = arma::norm(state.rows(VX, VZ)) + std::numeric_limits<double>::epsilon();
                ...
                // Calculate our new velocity based on our acceleration
                state.rows(VX, VZ) += state.rows(AX, AZ) * deltaT;
            ...
            arma::vec3 IMUModel::predictedObservation(const arma::vec::fixed<size>& state, const MeasurementType::ACCELEROMETER&) {
                ...
                // Add in our translational acceleration into the estimate
                down += state.rows(AX, AZ);
